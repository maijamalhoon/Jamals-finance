...
export default function SpendRecordWidget({
  ...
  const displaySpend = dailySpend.replace(/^PKR\s+/, "PKR");
  ...
  return (
    ...
    >
      <div className="flex min-h-0 flex-1 flex-col">
        <div>
          <p className="text-[24px] font-semibold leading-none text-[var(--dashboard-chart-spend)]">
            <CountedAmount amount={displaySpend} />
          </p>
          <p className="mt-1 text-[13px] leading-none text-text-secondary">
            daily average
          </p>
        </div>
        ...
        {hasSpendData ? (
              ...
              >
                <defs>
                  <linearGradient id="spendRecordFill" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="var(--dashboard-chart-spend)" stopOpacity={0.2} />
                    <stop offset="100%" stopColor="var(--dashboard-chart-spend)" stopOpacity={0.02} />
                  </linearGradient>
                </defs>
                <XAxis dataKey="day" hide />
                <YAxis hide domain={[0, "dataMax"]} />
                <Tooltip
                  cursor={{ stroke: "var(--dashboard-chart-grid)", strokeDasharray: "4 5" }}
                  contentStyle={{
          ...
          <div className="dashboard-chart-empty mt-auto h-[120px] min-h-[120px]">
            <div>
