alter table public.transactions
  add column if not exists investment_id uuid
    references public.investments(id) on delete cascade;

create unique index if not exists transactions_investment_id_unique_idx
  on public.transactions(investment_id)
  where investment_id is not null;

create index if not exists transactions_user_investment_idx
...
select users.id, 'Investments', 'expense', '#f59e0b'
from users
where not exists (
  select 1
  from public.categories category
  where category.user_id = users.id
...
begin
  ...
  if investment_category_id is null then
    insert into public.categories(user_id, name, type, color)
    values (current_user_id, 'Investments', 'expense', '#f59e0b')
  ...
  if saved_transaction_id is null then
    ...
    set
      ...
      date = p_purchased_at,
      note = 'Investment buy: ' || btrim(p_name),
