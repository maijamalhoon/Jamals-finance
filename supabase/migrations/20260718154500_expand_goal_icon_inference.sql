...
as $$
  select case
    when normalized ~ '\m(bike|bikes|motorbike|motorcycle|scooter|yamaha|cd70|cg125)\M'
      ...
      then 'bike'
    when normalized ~ '\m(car|cars|vehicle|auto|gari|gaari|toyota|suzuki|honda|hyundai|kia|nissan|tesla|bmw|mercedes|audi|lexus|ford|chevrolet|porsche|ferrari|lamborghini|civic|corolla|markx|alto|swift|fortuner|hilux|vitz|prius|cultus|mehran)\M'
    ...
    when normalized ~ '\m(renovation|construction|remodel|interior)\M'
      ...
      then 'hammer'
    when normalized ~ '\m(home|house|ghar|makan|makaan|apartment|flat|property|plot)\M'
      then 'home'
