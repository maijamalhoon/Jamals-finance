...
function sanitizeTransactions(rows: RawTransaction[]) {
  return rows.flatMap<AnalyticsTransactionData>((row) => {
    ...
    return [{
      ...
      amount,
      date,
      categoryId: category?.id?.trim() || row.category_id?.trim() || "uncategorized",
...
function SummaryValue({
  ...
  return (
    <p className={`mt-2 break-words text-xl font-bold ${tone ?? "text-text-primary"}`}>
...
export default async function ReportsPage({
  ...
  const [
    ...
    supabase
      .from("transactions")
      .select(
        "id, type, amount, date, category_id, account_id, source_name, person_name, item_name, categories(id, name, color), accounts(id, name, type)",
      )
    ...
    supabase
      ...
      .select(
        "id, amount, from_account:from_account_id(id, name, type), to_account:to_account_id(id, name, type)",
      )
    ...
    supabase.from("liabilities").select("id, remaining_amount, due_date, status"),
    supabase.from("investments").select("id, quantity, purchase_price, current_price"),
  ]);
  ...
  const cashFlow = buildCashFlowSeriesForRange(transactions, range).map((point) => ({
    month: point.label,
    income: point.income,
    expenses: point.expenses,
  }));
  const categories = buildExpenseCategoryBreakdown(transactions, range).map((item) => ({
    name: item.name,
    amount: item.amount,
    color: item.color,
    pct: item.percentage,
  ...
  const ensureAccount = (id: string, name: string) => {
    ...
    const next = { id, name, income: 0, expenses: 0, transfersIn: 0, transfersOut: 0 };
  ...
  transactions.forEach((transaction) => {
    ...
    if (!id) return;
    const account = ensureAccount(id, transaction.accountName?.trim() || "Unknown account");
    if (transaction.type === "income") account.income += positiveNumber(transaction.amount);
    if (transaction.type === "expense") account.expenses += positiveNumber(transaction.amount);
    if (transaction.type === "refund") account.expenses -= positiveNumber(transaction.amount);
  });
  if (!transfersResult.error) {
    ((transfersResult.data ?? []) as RawTransfer[]).forEach((transfer) => {
      ...
      if (amount <= 0) return;
      if (from?.id) {
        ensureAccount(from.id, from.name?.trim() || "From account").transfersOut += amount;
      }
      if (to?.id) {
        ensureAccount(to.id, to.name?.trim() || "To account").transfersIn += amount;
      }
    });
  }
  const accountRows = Array.from(accountSummary.values()).sort((left, right) => {
    const leftActivity = left.income + left.expenses + left.transfersIn + left.transfersOut;
    const rightActivity = right.income + right.expenses + right.transfersIn + right.transfersOut;
    return rightActivity - leftActivity || left.name.localeCompare(right.name);
  });

  const goalRows = goalsResult.error ? [] : (goalsResult.data ?? []);
  const goalTarget = goalRows.reduce((sum, goal) => sum + positiveNumber(goal.target_amount), 0);
  const goalProgress = goalRows.reduce((sum, goal) => sum + Math.max(positiveNumber(goal.current_amount), 0), 0);
  ...
  const payableRemaining = payableRows.reduce(
    (sum, payable) => sum + Math.max(positiveNumber(payable.remaining_amount), 0),
    0,
  );
  ...
  const investmentRows = investmentsResult.error ? [] : (investmentsResult.data ?? []);
  ...
  const transferCount = transfersResult.error ? null : transfersResult.data?.length ?? 0;

  return (
    <div data-print-report className="space-y-5 print:space-y-4">
      <div className="page-heading finance-surface-glass overflow-hidden print:border-none print:bg-white print:shadow-none">
        <div className="min-w-0">
          ...
          <p className="page-subtitle">
            {rangeLabel} · {entryCount} income and expense {entryCount === 1 ? "entry" : "entries"}
            {refundCount > 0 ? ` · ${refundCount} ${refundCount === 1 ? "refund" : "refunds"}` : ""}
            {transferCount === null ? "" : ` · ${transferCount} ${transferCount === 1 ? "transfer" : "transfers"}`}
          </p>
        </div>
      ...
      {parsed.wasReset || invalidRequestedPeriod ? (
        <InlineNotice tone="warning" role="status" className="print:hidden">
          The requested report range was invalid, so the current month was loaded instead.
        </InlineNotice>
      ) : null}

      {!financialDataAvailable ? (
        <InlineNotice tone="danger" role="alert">
          This report could not be prepared. Check your connection and try again; saved records were not changed.
        </InlineNotice>
      ) : partialAreas.length > 0 ? (
        <InlineNotice tone="warning" role="status">
          Report prepared with partial data. Unavailable sections: {partialAreas.join(", ")}.
        </InlineNotice>
      ) : null}
      ...
      <div data-mobile-summary-grid className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <article className="summary-card min-w-0">
          ...
          <SummaryValue amount={income} available={financialDataAvailable} tone="text-success" />
        </article>
        <article className="summary-card min-w-0">
          ...
          <SummaryValue amount={expenses} available={financialDataAvailable} tone="text-danger" />
        </article>
        <article className="summary-card min-w-0">
        ...
        <article className="summary-card min-w-0">
          <div className="flex items-center gap-2 text-info">
            ...
            <p className="text-xs font-semibold text-text-secondary">Daily averages</p>
          </div>
          <p className="mt-2 text-sm font-bold text-text-primary">
            {financialDataAvailable ? <Money amount={periodFacts.averageDailySpending} /> : "Unavailable"}
          </p>
          <p className="mt-1 text-xs text-text-secondary">spending per day · {periodFacts.inclusiveDays} days</p>
        </article>
      </div>

      {financialDataAvailable && entryCount === 0 && transferCount === 0 ? (
        <div className="finance-panel p-5 text-center sm:p-7">
          <FileBarChart aria-hidden="true" className="mx-auto size-8 text-text-secondary" />
          <h2 className="mt-3 text-sm font-semibold text-text-primary">No report activity in this range</h2>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className="finance-panel min-w-0 p-4 sm:p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-text-primary">Income sources</h2>
          </div>
          {incomeSources.items.length === 0 ? (
            <p className="py-8 text-center text-sm text-text-secondary">No income sources in this range.</p>
          ) : (
            <div className="space-y-3">
              {incomeSources.items.slice(0, 8).map((source) => (
                <div key={source.id} className="finance-panel-soft flex min-w-0 items-center justify-between gap-3 p-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-text-primary">{source.name}</p>
                    <p className="mt-0.5 text-xs text-text-secondary">{source.percentage.toFixed(1)}% of income</p>
                  </div>
                  <p className="shrink-0 text-sm font-bold text-success"><Money amount={source.amount} /></p>
                </div>
              ))}
            </div>
          )}
        ...
        <section className="finance-panel min-w-0 p-4 sm:p-5">
          <div className="mb-4 flex items-center gap-2">
            <WalletCards aria-hidden="true" size={16} className="text-info" />
            <h2 className="text-sm font-semibold text-text-primary">Account activity</h2>
          </div>
          {accountRows.length === 0 ? (
            <p className="py-8 text-center text-sm text-text-secondary">No account activity in this range.</p>
          ) : (
            <div className="space-y-3">
              {accountRows.slice(0, 8).map((account) => (
                <article key={account.id} className="finance-panel-soft min-w-0 p-3">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-xs sm:grid-cols-4">
                    <span className="text-success">In <Money amount={account.income} /></span>
                    <span className="text-danger">Out <Money amount={account.expenses} /></span>
                    <span className="text-info">Transfer in <Money amount={account.transfersIn} /></span>
                    <span className="text-info">Transfer out <Money amount={account.transfersOut} /></span>
                  </div>
                </article>
              ))}
            </div>
          ...
          {expenseAccounts.length > 0 ? (
            <p className="mt-3 text-xs text-text-secondary">
              Highest spending account: {expenseAccounts[0].name} ({expenseAccounts[0].percentage.toFixed(1)}%).
            </p>
          ) : null}
        </section>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <section className="finance-panel min-w-0 p-4 sm:p-5">
          <div className="flex items-center gap-2 text-goals">
            ...
            <h2 className="text-sm font-semibold text-text-primary">Goals overview</h2>
          </div>
          {goalsResult.error ? (
            <p className="mt-5 text-sm text-text-secondary">Goal data unavailable.</p>
          ) : (
            <>
              <p className="mt-5 text-xl font-bold text-text-primary"><Money amount={goalProgress} /></p>
              <p className="mt-1 text-xs text-text-secondary">toward <Money amount={goalTarget} /></p>
              <p className="mt-3 text-xs font-semibold text-goals">{completedGoals} of {goalRows.length} completed</p>
            </>
          )}
        </section>
        ...
        <section className="finance-panel min-w-0 p-4 sm:p-5">
          <div className="flex items-center gap-2 text-payables">
            ...
            <h2 className="text-sm font-semibold text-text-primary">Payables overview</h2>
          </div>
          {payablesResult.error ? (
            <p className="mt-5 text-sm text-text-secondary">Payable data unavailable.</p>
          ) : (
            <>
              <p className="mt-5 text-xl font-bold text-payables"><Money amount={payableRemaining} /></p>
              <p className="mt-1 text-xs text-text-secondary">remaining across {payableRows.length} payables</p>
              <p className={`mt-3 text-xs font-semibold ${overduePayables > 0 ? "text-danger" : "text-success"}`}>
        ...
        <section className="finance-panel min-w-0 p-4 sm:p-5">
          <div className="flex items-center gap-2 text-investment">
            ...
            <h2 className="text-sm font-semibold text-text-primary">Investment overview</h2>
          </div>
          {investmentsResult.error ? (
            <p className="mt-5 text-sm text-text-secondary">Investment data unavailable.</p>
          ) : (
            <>
              <p className="mt-5 text-xl font-bold text-investment"><Money amount={investmentSummary.current} /></p>
              <p className="mt-1 text-xs text-text-secondary">current value · invested <Money amount={investmentSummary.invested} /></p>
              {investmentSummary.partial ? (
                <p className="mt-3 text-xs font-semibold text-warning">
                  Market result unavailable · partial pricing
                </p>
              ) : (
                <p className={`mt-3 text-xs font-semibold ${investmentPnl > 0 ? "text-success" : investmentPnl < 0 ? "text-danger" : "text-text-secondary"}`}>
                  Market {investmentPnl > 0 ? "gain" : investmentPnl < 0 ? "loss" : "change"}: <Money amount={Math.abs(investmentPnl)} />
                </p>
              )}
            </>
          )}
