...
export default function SimpleShopError({
  reset,
}: {
    ...
    <main className="min-h-dvh bg-background px-4 py-8 text-foreground sm:px-6 lg:px-8">
      <section className="mx-auto flex min-h-[60vh] w-full max-w-xl items-center justify-center">
        <div className="w-full rounded-[var(--radius-card)] bg-surface px-5 py-7 text-center shadow-[var(--shadow-sm)] sm:px-7">
          ...
          <h1 className="mt-4 text-xl font-black text-text-primary">Simple Shop could not be loaded</h1>
          <p className="mt-2 text-sm leading-6 text-text-secondary">
            No sale, purchase, stock movement, expense, cash entry, or accounting journal was changed.
          </p>
          <Button type="button" className="mt-5" onClick={reset}>
            <RotateCcw aria-hidden="true" /> Retry shop
          </Button>
        </div>
      </section>
    </main>
