export default function BusinessApprovalsLoading() {
  return (
    <main className="min-h-dvh bg-background px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto w-full max-w-7xl animate-pulse">
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {Array.from({ length: 5 }, (_, index) => <div key={index} className="h-28 rounded-[var(--radius-card)] bg-surface" />)}
        </div>
        <div className="mt-8 space-y-4">
          {Array.from({ length: 3 }, (_, index) => <div key={index} className="h-64 rounded-[var(--radius-card)] bg-surface" />)}
        </div>
      </div>
    </main>
  );
