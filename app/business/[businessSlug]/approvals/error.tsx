...
export default function BusinessApprovalsError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <main className="grid min-h-dvh place-items-center bg-background px-4 py-10 text-foreground">
      <section className="w-full max-w-md rounded-[var(--radius-card)] bg-surface px-6 py-8 text-center shadow-[var(--shadow-sm)]">
        ...
        <h1 className="mt-5 text-xl font-black text-text-primary">Approvals could not be loaded</h1>
        <p className="mt-3 text-sm leading-6 text-text-secondary">No policy, request, assignment, decision, or audit record was changed. Retry the protected workspace request.</p>
