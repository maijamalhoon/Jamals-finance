...
import BusinessBankingWorkspace, {
  type BusinessBankingSnapshot,
} from "@/components/business/BusinessBankingWorkspace";
...
export default async function BusinessBankingPage({
  ...
  const currentPath = `/business/${businessSlug}/banking`;
  ...
  if (businessResult.error) {
    console.error("Banking business load failed", { code: businessResult.error.code });
  }
  ...
  const snapshotResult = await supabase.rpc("get_business_banking_snapshot", {
    p_business_id: business.id,
    p_bank_account_id: validUuid(query.account),
    p_start_date: start,
    p_end_date: end,
    p_line_limit: 250,
  });
  if (snapshotResult.error) {
    console.error("Banking snapshot failed", { code: snapshotResult.error.code });
    throw new Error("Banking and reconciliation data could not be loaded.");
  }
  ...
  return (
    <>
      ...
      <BusinessBankingWorkspace
        ...
        initialStart={start}
        initialEnd={end}
        snapshot={(snapshotResult.data ?? {}) as BusinessBankingSnapshot}
      />
    </>
  );
}
