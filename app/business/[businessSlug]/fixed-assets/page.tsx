...
export default async function BusinessFixedAssetsPage({
  params,
}: {
  ...
  const currentPath = `/business/${businessSlug}/fixed-assets`;
  ...
  if (businessResult.error) {
    console.error("Fixed asset business load failed", { code: businessResult.error.code });
  }
  ...
  const canView =
    ...
    permissions.includes("assets.depreciate") ||
    permissions.includes("assets.dispose");

  if (!canView) notFound();

  const snapshotResult = await supabase.rpc("get_business_assets_snapshot", {
    p_business_id: business.id,
    p_limit: 200,
  });

  if (snapshotResult.error) {
    console.error("Fixed asset snapshot failed", { code: snapshotResult.error.code });
    throw new Error("Fixed asset data could not be loaded.");
  }
  ...
  return (
    <>
      ...
      <BusinessFixedAssetsWorkspace
        businessId={business.id}
        businessName={business.name}
        baseCurrency={business.base_currency}
        today={dateInZone(business.timezone)}
        snapshot={(snapshotResult.data ?? {}) as BusinessAssetsSnapshot}
      />
    </>
  );
}
