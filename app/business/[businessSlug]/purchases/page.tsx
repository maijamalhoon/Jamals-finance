...
function statusClass(status: SupplierBillRow["status"], overdue: boolean) {
...
export default async function BusinessPurchasesPage({
  ...
  const canPaySuppliers =
    canManagePurchases || permissions.includes("purchases.pay");
  ...
  const [settingsResult, suppliersResult, accountsResult, billsResult] = await Promise.all([
    ...
    supabase
      ...
      .select(
        "id, supplier_id, bill_code, supplier_document_number, bill_date, due_date, status, currency, exchange_rate, total_transaction, paid_transaction, total_base, paid_base, journal_entry_id, issued_at",
      )
      .eq("business_id", business.id)
  ...
  const openBills = bills
    ...
    .map((bill) => ({
      ...
      outstanding: Math.max(0, Number(bill.total_transaction) - Number(bill.paid_transaction)),
  ...
  const outstandingBase = Math.max(0, totalPurchasedBase - totalPaidBase);
  const overdueCount = bills.filter(
    (bill) => bill.status !== "paid" && bill.due_date < today,
  ).length;

  const loadError =
    settingsResult.error || suppliersResult.error || accountsResult.error || billsResult.error;
      ...
      <div className="mx-auto w-full max-w-7xl">
        ...
        <header className="mt-7">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-7 text-text-secondary sm:text-base">
            Supplier bills, discounts, recoverable taxes, expense or asset allocations, partial
            settlements, and Accounts Payable journals use one atomic tenant-scoped workflow.
          </p>
        </header>

        {loadError ? (
          <section className="mt-6 rounded-[var(--radius-card)] bg-danger-soft px-4 py-4 text-sm text-danger">
            Some purchase information could not be loaded. No supplier bill, payment, or journal was changed.
          </section>
        ) : null}

        {!accrualReady ? (
          <section className="mt-6 rounded-[var(--radius-card)] bg-warning-soft px-4 py-4 text-sm text-warning">
            Automated supplier bill posting is intentionally paused because this company is not using
            accrual accounting. Cash-basis purchasing requires a separate verified recognition workflow.
          </section>
        ) : null}
        ...
        <section className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <article className="rounded-[var(--radius-card)] bg-surface px-5 py-5 shadow-[var(--shadow-sm)]">
            ...
            <p className="mt-4 text-xs font-bold text-text-secondary">Purchased</p>
            <strong className="mt-1 block truncate text-lg font-black text-text-primary">
              {formatMoney(totalPurchasedBase, business.base_currency)}
            </strong>
          </article>
          <article className="rounded-[var(--radius-card)] bg-surface px-5 py-5 shadow-[var(--shadow-sm)]">
          ...
          suppliers.length > 0 && allocationAccounts.length > 0 ? (
            ...
            <section className="mt-8 rounded-[var(--radius-card)] bg-surface-secondary px-5 py-6">
              ...
              <p className="mt-1 text-sm text-text-secondary">
                A posted supplier bill requires an active tenant-scoped supplier and a valid expense,
                inventory, prepaid, or fixed-asset allocation account.
              </p>
              <Link
        ...
        <section className="mt-9">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-primary">Payables register</p>
          ...
          {bills.length > 0 ? (
            <div className="mt-4 overflow-x-auto rounded-[var(--radius-card)] bg-surface shadow-[var(--shadow-sm)]">
              <table className="w-full min-w-[900px] text-left text-sm">
                ...
                <tbody>
                  {bills.map((bill) => {
                    const overdue = bill.status !== "paid" && bill.due_date < today;
                    const outstanding = Math.max(
                      0,
                      Number(bill.total_transaction) - Number(bill.paid_transaction),
                    );
                    return (
                      <tr key={bill.id} className="border-t border-border-subtle/60">
                        ...
                        <td className="px-4 py-3 text-text-secondary">{formatDate(bill.due_date)}</td>
                        <td className="px-4 py-3">
                          ...
                          >
                            {overdue ? "Overdue" : formatLabel(bill.status)}
                          </span>
                        </td>
            ...
            <div className="mt-4 rounded-[var(--radius-card)] bg-surface-secondary px-5 py-8 text-center">
              ...
              <p className="mt-1 text-sm text-text-secondary">
                Issued supplier bills will appear here with payment and overdue status.
              </p>
            </div>
          )}
        </section>
